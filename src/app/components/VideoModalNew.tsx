...
export default function VideoModalNew({
  videoDetails,
  className = "",
  isPopup = false,
  onClose,
    ...
    >
      <div className="w-full max-w-[750px] px-4" ref={toggleRef}>
        ...
        >
          {isPopup && onClose && (
            <button
              className="absolute -top-10 -right-[5px] md:-right-5  hover:text-gray-800 transition-colors duration-200  rounded-full"
              onClick={onClose}
              aria-label="Close video"
            >
              <CloseIcon color="white" className="size-5"/>
            </button>
          )}
